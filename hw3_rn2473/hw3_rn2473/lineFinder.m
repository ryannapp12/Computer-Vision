function line_detected_img = lineFinder(orig_img, hough_img, hough_threshold)
    % We are creating a copy of the input image
    line_detected_img = orig_img;
    % Check which values are greater the threshold
    over_thresh = hough_img > hough_threshold;
    % Finding elements which are the greatest in neighborhood
    maxs_in_nbhd = findMaxsInNbhd(hough_img, 9);
    % Defining theta and rho vectors
    thetas = linspace(-pi/2, pi/2, size(hough_img, 2));
    img_diagonal = round(sqrt(size(orig_img, 1)^2 + size(orig_img, 2)^2));
    rhos = linspace(-img_diagonal, img_diagonal, size(hough_img, 1));
    x = 1:size(orig_img, 2);
    for i=1:size(hough_img, 1)
        for j=1:size(hough_img, 2)
            if over_thresh(i, j) == 1 && maxs_in_nbhd(i, j) == 1
                theta = thetas(j);
                rho = rhos(i);
                y = round((rho - x*sin(theta))/cos(theta));
                % We are keeping only points which belong to the visible
                % part of the image
                x_plot = x(y>=1 & y<=size(orig_img, 2));
                y_plot = y(y>=1 & y<=size(orig_img, 2));
                for seg=1:length(x_plot)-1
                    % insertShape was used as an alternative to the method
                    % proposed in the demoMATLABTricksFun.m for saving
                    % annotated image
                    line_detected_img = insertShape(line_detected_img,'Line',[x_plot(seg) y_plot(seg) x_plot(seg+1) y_plot(seg+1)],'LineWidth',2,'Color','red');
                end
            end
        end
    end
end
